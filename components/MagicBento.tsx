...
export interface BentoProps {
  ...
  renderCardContent?: (card: BentoCardProps, index: number, textAutoHide: boolean) => React.ReactNode;
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className={`bento-section grid gap-2 p-3 select-none relative w-full ${className}`}
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const data = (cards && cards.length > 0 ? cards : cardData).slice(0, 6);
    ...
    <>
      <style>
        {`
          ...
          /* Ensure icons inherit text color in light/dark modes */
          .card svg, .card [data-lucide-icon], .card .icon {
            color: currentColor;
          }
          ...
          {data.map((card, index) => {
            ...
            if (enableStars) {
              return (
                ...
                >
                  {renderCardContent ? renderCardContent(card, index, !!textAutoHide) : defaultContent}
                </ParticleCard>
              );
            }
            ...
            return (
              ...
              >
                {renderCardContent ? renderCardContent(card, index, !!textAutoHide) : (
                  <>
                    <div className="card__header flex justify-between gap-3 relative">
                      <span className="card__label text-base">{card.label}</span>
                    </div>
